...
// Import the exact same niches seen in the user dashboard
...
export default function AdminModelsPage() {
  const [models, setModels] = useState(mockNiches);
  const [editingModel, setEditingModel] = useState<typeof mockNiches[0] | null>(null);
  ...
  return (
    ...
    >
      <motion.div variants={fadeUp} className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          <Button 
            ...
            disabled={isSyncing}
            variant="outline" 
            className="bg-white border-zinc-200 text-black hover:bg-zinc-50 rounded-xl shadow-sm h-10 font-bold px-4 transition-all"
          >
          ...
          <DialogTrigger
            render={
              <div className="inline-flex items-center justify-center font-bold whitespace-nowrap bg-black hover:bg-zinc-800 text-white rounded-xl shadow-lg shadow-black/10 gap-2 px-6 h-10 text-sm cursor-pointer transition-colors">
          ...
          <DialogContent className="bg-white border-black/5 text-zinc-900 max-w-lg rounded-3xl shadow-2xl">
            ...
            <form className="space-y-4 mt-2" onSubmit={(e) => e.preventDefault()}>
              <div className="space-y-1.5">
                ...
                <Input placeholder="e.g. Website Builder" className="bg-zinc-50 border-zinc-200 rounded-xl" />
              </div>
              <div className="space-y-1.5">
                <Label className="font-bold text-zinc-600">Provider Model ID (hidden from users)</Label>
                <Input placeholder="e.g. moonshotai/moonshot-v1-auto" className="bg-zinc-50 border-zinc-200 rounded-xl" />
              </div>
              <div className="space-y-1.5">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  ...
                  <Input type="number" disabled placeholder="Auto-computed" className="bg-zinc-100 border-zinc-200 rounded-xl text-zinc-400 font-mono" />
                ...
                <div className="space-y-1.5">
                  ...
                  <Input type="number" placeholder="50" className="bg-zinc-50 border-zinc-200 rounded-xl" />
                </div>
              </div>
              <Button type="button" className="w-full bg-black hover:bg-zinc-800 text-white rounded-xl font-bold py-6 mt-4">Save Niche Tool</Button>
            </form>
          </DialogContent>
        </Dialog>
        </div>
        ...
        <DialogContent className="bg-white border-black/5 text-zinc-900 max-w-lg rounded-3xl shadow-2xl">
          ...
          {editingModel && (
            <form className="space-y-4 mt-2" onSubmit={saveEdit}>
              ...
              <Button type="submit" className="w-full bg-black hover:bg-zinc-800 text-white rounded-xl font-bold py-6 mt-4">Save Changes</Button>
            </form>
          )}
        </DialogContent>
      </Dialog>
      ...
      <div className="space-y-4">
        {models.map((model) => {
          const baseCost = parseFloat((model.baseCostPer1k).toString());
          const effectiveCost = (baseCost * (1 + model.profitMarginPercent / 100)).toFixed(4);
          
          return (
            <motion.div key={model.id} variants={fadeUp}>
              <Card className={`bg-white/60 backdrop-blur-2xl border-none shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:bg-white/80 transition-all duration-500 overflow-hidden ${!model.isActive ? 'opacity-60 grayscale-[0.5]' : ''}`}>
                <CardContent className="p-0">
                  <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 p-6">
                    <div className="flex items-center gap-5">
                      <div className={`p-3 rounded-2xl shadow-sm border ${model.isActive ? 'bg-emerald-50 border-emerald-100/50' : 'bg-zinc-100 border-zinc-200'}`}>
                        <BrainCircuit className={`h-6 w-6 ${model.isActive ? 'text-emerald-600' : 'text-zinc-500'}`} />
                      </div>
                      <div>
                        <div className="flex items-center gap-3">
                          <h3 className="font-black text-xl text-black tracking-tight">{model.nicheName}</h3>
                          <Badge variant={model.isActive ? "default" : "secondary"} className={model.isActive ? "bg-emerald-100 text-emerald-700 hover:bg-emerald-200 border-none px-2 shadow-sm font-bold" : "bg-zinc-200 text-zinc-600 hover:bg-zinc-300 border-none font-bold"}>
                            {model.isActive ? "Active" : "Disabled"}
                          </Badge>
                        </div>
                        <p className="text-sm font-medium text-zinc-500 mt-1">Model: <span className="text-zinc-600 font-mono tracking-tight">{model.providerModelId}</span></p>
                      </div>
                    </div>
                    
                    <div className="flex items-center gap-8 text-sm shrink-0 bg-zinc-50/50 rounded-2xl py-3 px-6 border border-black/5">
                      <div className="text-right">
                        <p className="text-zinc-500 text-xs font-bold uppercase tracking-widest mb-1">Base Cost / 1k</p>
                        <p className="font-mono text-zinc-900 font-medium">${baseCost.toFixed(4)}</p>
                        <p className="text-[10px] text-zinc-400/80 mt-1 font-medium tracking-tight whitespace-nowrap">{formatSyncDate((model as any).lastSyncedAt)}</p>
                      </div>
                      <div className="text-right">
                        <p className="text-zinc-500 text-xs font-bold uppercase tracking-widest mb-1">Profit Margin</p>
                        <p className="font-mono text-violet-600 font-bold">+{model.profitMarginPercent}%</p>
                      </div>
                      <div className="text-right pr-4 border-r border-black/5">
                        <p className="text-zinc-500 text-xs font-bold uppercase tracking-widest mb-1">User Price / 1k</p>
                      ...
                      <div className="flex gap-1 pl-2">
                        ...
                        <Button 
                          ...
                          className={`h-10 w-10 rounded-xl transition-colors ${model.isActive ? 'text-red-400 hover:text-red-600 hover:bg-red-50' : 'text-emerald-500 hover:text-emerald-700 hover:bg-emerald-50'}`}
                          title={model.isActive ? 'Disable Tool' : 'Enable Tool'}
                        >

...
export function UsageClient({ initialLogs, initialMetrics }: { initialLogs: any[], initialMetrics: any }) {
      ...
      <motion.div variants={fadeUp} className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <Card className="bg-white/60 backdrop-blur-3xl border-none shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:bg-white/80 transition-colors">
          <CardContent className="p-6">
            ...
            <p className="text-4xl font-black text-black mt-2 tracking-tighter">{formatValue(initialMetrics?.totalCalls)}</p>
            <p className="text-xs font-bold text-emerald-600 mt-2 bg-emerald-50 inline-block px-2 py-1 rounded-md">Live from DB</p>
          </CardContent>
        </Card>
        <Card className="bg-white/60 backdrop-blur-3xl border-none shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:bg-white/80 transition-colors">
          <CardContent className="p-6">
            ...
            <p className="text-4xl font-black text-black mt-2 tracking-tighter">{formatValue(initialMetrics?.totalTokens)} <span className="text-xl text-zinc-400 font-medium">tokens</span></p>
          </CardContent>
        </Card>
        <Card className="bg-white/60 backdrop-blur-3xl border-none shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:bg-white/80 transition-colors">
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <p className="font-bold text-zinc-500 uppercase tracking-widest text-xs">Gross Revenue</p>
              <ArrowUpRight className="h-4 w-4 text-emerald-500" />
            </div>
            <p className="text-4xl font-black text-black mt-2 tracking-tighter">${(initialMetrics?.grossRevenue || 0).toFixed(2)}</p>
          </CardContent>
        </Card>
      </motion.div>
              ...
              {filteredLogs.length === 0 ? (
                <tr>
                  <td colSpan={7} className="px-6 py-8 text-center text-zinc-500 font-medium">
                    No usage logs found in the database.
                  </td>
                </tr>
                ...
                >
                  <td className="px-6 py-4 font-mono text-[10px] text-zinc-500 font-medium max-w-[120px] truncate" title={log.id}>
                    {log.id}
                  </td>

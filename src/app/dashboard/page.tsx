...
const niches = [
  { id: "website-builder", name: "Website Builder", icon: Globe },
  { id: "content-writer", name: "Content Writer", icon: FileText },
  { id: "deep-research", name: "Deep Research", icon: Zap },
  { id: "code-generator", name: "Expert Developer", icon: Terminal },
  { id: "image-crafter", name: "Image Crafter", icon: Image },
];
...
export default function DashboardPage() {
  ...
  const handleSearchSubmit = (e: React.FormEvent) => {
    ...
    router.push(`/dashboard/chat/deep-research?q=${encodeURIComponent(prompt)}`);
  };

  const handleAttachClick = () => {
    // 1. Auth Guard (Mock simple - eventually use real clerk state)
    const isLoggedIn = true; // Simulating logged in locally
    if (!isLoggedIn) {
    ...
    <div className="h-full flex flex-col items-center justify-center px-4 -mt-20">
      <div className="w-full max-w-2xl text-center space-y-8">
        ...
        <div className="flex flex-wrap items-center justify-center gap-2">
          ...
          <Link href="/dashboard/chat/agent-swarm" className="flex items-center gap-2 px-4 py-2 bg-zinc-900 text-white rounded-full text-sm font-medium hover:bg-zinc-800 transition-all">
            Agent Swarm
          </Link>
        </div>
      </div>

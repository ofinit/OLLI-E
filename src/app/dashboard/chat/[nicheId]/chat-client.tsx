...
export function ChatClient({ nicheId, niche, isS3Configured: initialS3, walletBalance }: {
  ...
  const exportAsText = (content: string) => {
    ...
    a.href = URL.createObjectURL(blob); a.download = `olli-e-${Date.now()}.txt`; a.click();
  };
  ...
  const exportAsHTML = (content: string) => {
    ...
    const a = document.createElement("a");
    a.href = URL.createObjectURL(blob); a.download = `olli-e-${Date.now()}.html`; a.click();
  };
                ...
                <div className="flex flex-col gap-2 max-w-[85%]">
                  ...
                  {m.role === "assistant" && (
                    <div className="flex gap-1">
                      ...
                      <Button size="sm" variant="ghost" className="h-8 text-[11px] text-zinc-400 hover:text-zinc-900 hover:bg-zinc-50 gap-1.5 px-2" onClick={() => navigator.clipboard.writeText(m.content)}>

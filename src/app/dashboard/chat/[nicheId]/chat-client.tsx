...
export function ChatClient({ 
  ...
  const exportAsHTML = (content: string) => {
    // Convert basic markdown to HTML for a proper rich export
    const body = content
      ...
      .replace(/(<li>.*<\/li>\n?)+/g, (m) => `<ul style="padding-left:1.5rem;margin:8px 0">${m}</ul>`)
      .replace(/\n\n/g, '</p><p>')
      .replace(/\n/g, '<br/>');
...
<head>
  ...
  <style>
    body{font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;max-width:800px;margin:3rem auto;padding:0 2rem;color:#111;line-height:1.7}
    h1,h2,h3{font-weight:700;margin:1.5rem 0 0.5rem}
    p{margin:0.75rem 0}
